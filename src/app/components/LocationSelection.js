...
const LocationSelection = () => {
  const [location, setLocation] = useState('Selcect Location')
  return (
    <Menu as='div' className='w-full h-full flex xl:flex-row'>
      <div className='relative flex-1'>
        {/* btn */}
        <Menu.Button className='dropdown-btn w-full h-full flex-col
...
<div className='text-[15px] uppercase font-bold'>Select Location</div> 
        ...
        {/* items */}
        <Menu.Items className='drodown-menu shadow-lg absulote -top-56 xl:top-[90px]
        left-1/2 xl:left-0 z-10 transform -translate-x-1/2 xl:-translate-x-0 text-sm
        text-center xl:text-left w-full bg-white max-w-[332px] py-6 rounded-[10px]'>Menu
         {locations.map((location,index) => {
          return <div
          onClick={() => setLocation(location)}
          key={index} className='cursor-pointer py-4 xl:pl-10 hover:bg-gray-50 text-[13px] uppercase'>{location}</div>
         })}
